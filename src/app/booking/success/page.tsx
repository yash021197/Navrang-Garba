...
type BookingView = { payment_status: string; quantity: number; amount: number | string; customers: { name: string; email: string | null }[]; event_days: { event_date: string }[]; ticket_types: { name: string }[] };
...
export default async function BookingSuccessPage({ searchParams }: PageProps<"/booking/success">) {
  ...
  const paidTicket = paid && booking && ticket ? <TicketCard ticketReference={ticket.ticket_reference} date={booking.event_days[0]?.event_date ?? ""} ticketType={booking.ticket_types[0]?.name ?? "Pass"} quantity={booking.quantity} amount={Number(booking.amount)} customerName={booking.customers[0]?.name ?? ""} /> : null;
