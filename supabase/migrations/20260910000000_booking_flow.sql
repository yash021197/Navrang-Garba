...
create unique index bookings_idempotency_key_idx on public.bookings (idempotency_key) where idempotency_key is not null;
create unique index customers_mobile_unique_idx on public.customers (mobile);

update public.event_days
set event_date = date '2026-10-10' + day_number, active = true;
...
begin
  ...
  new_reference := 'BOOK-' || upper(encode(gen_random_bytes(8), 'hex'));
  ...
  select id, 'RAZORPAY', amount, currency, 'PENDING' from public.bookings where booking_reference = new_reference;
